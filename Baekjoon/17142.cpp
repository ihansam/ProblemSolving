...
int infect_time(){
	...
	for(int i=1; i<=N; ++i){
		for(int j=1; j<=N; ++j){
			if(!copy_map[i][j]) return -1;
			else if(copy_map[i][j] > max_time && lab[i][j] != -2)
				max_time=copy_map[i][j];
		}
	}
	if(max_time == -1) return 0;
	else return max_time - 1;
}
...
void spread_virus_bfs(){
	...
	if(inf_time == -1) return;
	if((min_time == -1) || (inf_time < min_time)) min_time = inf_time;
}
